...
//Code Here
let evenNumbers = mixedNumbers.filter((number) => { return number % 2 === 0 })
...
//Code Here
let postTaxPrices = prices.map((price) => { return price + price * .07 });
...
//Code Here
let totalPopulation = populations.reduce((acc, population) => { return acc + population }, 0)
...
//Code Here
let myStrongest = monstersInYourPocket.filter((monster) => { return monster.CP > 200 })
...
let orderTotals = orders.map((order) => { return order.price + order.price * order.tax })
